...
from .workflow_records import build_result_lines, build_result_summary, dedupe_plan_records, record_dedupe_key, scan_duplicate_stats, undo_result_summary
...
class WorkflowController(QObject):
    ...
    def start_commit(self, records, target_dir, move=True, flat=False, no_px=False):
        if not self._engine:
            return
    ...
    def handle_commit_finished(self, res):
        ...
        summary_lines = build_result_lines(res) if isinstance(res, dict) else [summary]
        committed_count = 0
        if isinstance(res, dict):
    ...
    def handle_undo_finished(self, res):
        ...
        if isinstance(res, dict):
            ...
            if sources:
                self._persist_restored_source_after_undo(str(sources[0]), session_id=session_id)
            if sources and self._prompt_scan_restored_source(summary, str(sources[0])):
                return res
    ...
    def _persist_restored_source_after_undo(self, source: str, *, session_id: str = "") -> None:
        ...
        try:
            workflow_session_persistence.persist_restored_source(self.app.settings, source, session_id=session_id)
        except Exception:
    ...
    def _prompt_scan_restored_source(self, summary: str, source: str) -> bool:
        ...
        if not self._prompt_open_library_session(
            title="Undo Complete",
            summary=summary,
            prompt="Scan the restored source library now?",
            path=source,
            action_label="Scan Source",
        ):
            return False

        workflow_session_persistence.persist_restored_source_scan_target(self.app.settings, source)
        self.start_scan([source], append=False, last_target=source, require_clear_draft=False)
        return True
