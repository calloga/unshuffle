...
def append_refinement_candidates(conn: sqlite3.Connection, session_id: str, candidates: list[Any]) -> None:
    ...
    def upsert_coherence_review_decisions(self, session_id: str, decisions: list[dict[str, Any]]) -> None:
        ...
        self._connection.executemany(
            """
            INSERT INTO coherence_review_decisions (source_path, file_hash, decision_type,
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(source_path) DO UPDATE SET file_hash=excluded.file_hash,
    ...
    def upsert_anchor_candidates(self, session_id: str, anchors: list[Any]) -> None:
        self._connection.execute("DELETE FROM anchor_profiles WHERE session_id = ? AND state = 'candidate'",
                                 (session_id,))
        if not anchors:
            return
...
def append_anchor_candidates(conn: sqlite3.Connection, session_id: str, anchors: list[Any]) -> None:
    ...
    def upsert_anchor_profile_rows(self, session_id: str, rows: list[dict[str, Any]]) -> None:
        ...
        self._connection.executemany(
            """
            INSERT INTO anchor_profiles (session_id, anchor_id, audio_type, category, subcategory, cluster_id,
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(session_id, anchor_id) DO UPDATE SET audio_type=excluded.audio_type,
                                                             category=excluded.category,
                                                             subcategory=excluded.subcategory,
                                                             cluster_id=excluded.cluster_id,
    ...
    def remove_verified_anchor_profiles(self, session_id: str, anchor_ids: list[str]) -> None:
        ...
        self._connection.executemany(
            ...
            ON CONFLICT(session_id, anchor_id) DO UPDATE SET state='ignored',
                                                             updated_at=CURRENT_TIMESTAMP
            """,
